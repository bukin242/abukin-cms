...
//      $this->addToolbarSet($toolbarset);
  ...
  function getHtml()
  {
    ...
    if (SpawAgent::getAgent() != SPAW_AGENT_UNSUPPORTED)
    {
      ...
      {
        $pagetpl .= '<textarea name="'.$page->inputName.'" id="'.$pname.'" style="width: 100%; height: '.$this->height.'; display: none; overflow: scroll;" rows="10" cols="10">'.htmlspecialchars($page->value, ENT_COMPAT, 'CP1251').'</textarea>';
        $js_res .= 'var '.$pname.'_page = new SpawEditorPage("'.$pname.'","'.htmlspecialchars($page->caption, ENT_COMPAT, 'CP1251').'","'.$page->direction.'");'."\n";
    ...
    else
    {
      ...
      foreach($this->pages as $pname => $page)
      {
        ...
        $res .= '<textarea name="'.$pname.'" id="'.$pname.'" width="'.$this->width.'" height="'.$this->height.'" style="width: '.$this->width.'; height: '.$this->height.';" wrap="off">'.htmlspecialchars($page->value, ENT_COMPAT, 'CP1251').'</textarea><br />';
      }
    }
    return $res;
  }
